...
Briefly, TransportHealth allows user to upload the data of the study whose findings they wish to extend (the source data) and the data from the target population (the target data). Then, users can specify the covariates to adjust for in the source data, and what effect modifiers to adjust for between the source and study data. Finally, the results of the analysis will be displayed in tabular and graphical formats. TransportHealth supports the following statistical approaches to transportability analysis:
